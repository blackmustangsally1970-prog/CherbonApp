...
def wedding_hub(wedding_id):
    ...
    timeline = WeddingTimeline.query.filter_by(wedding_id=wedding.id).order_by(WeddingTimeline.created_at.desc()).all()

    # detail templates (for future client/caterer forms)
    detail_templates = WeddingDetailLibrary.query.filter_by(active=True).order_by(WeddingDetailLibrary.default_order.asc()).all()
